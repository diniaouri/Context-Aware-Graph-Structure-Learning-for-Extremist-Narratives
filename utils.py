...
def edge_deletion(adj, drop_r):
    ...
    samples = np.random.choice(num_edge, size=int(
        drop_r * num_edge), replace=False)
...
def edge_addition(adj, add_r):
    ...
    added_edge_index_in = np.random.choice(
        num_node, size=int(add_r * num_edge), replace=True)
    added_edge_index_out = np.random.choice(
        num_node, size=int(add_r * num_edge), replace=True)
...
def get_feat_mask(features, mask_rate):
    ...
    samples = np.random.choice(feat_node, size=int(
        feat_node * mask_rate), replace=False)
    mask[:, samples] = 1
...
def dgl_graph_to_torch_sparse(dgl_graph):
    ...
    indices = torch.cat(
        (torch.unsqueeze(rows_, 0), torch.unsqueeze(cols_, 0)), 0).cpu()
